...
@section('title', 'Добавить криптовалюту')
...
@section('content')
    ...
    <form action="{{ route('cryptocurrencies.store') }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <input type="number" name="network_difficulty" id="network_difficulty" class="form-control" value="{{ old('network_difficulty') }}" step="0.01" min="0" required>
        ...
        <div class="mb-3">
            ...
            <input type="number" name="block_reward" id="block_reward" class="form-control" value="{{ old('block_reward') }}" step="0.01" min="0" required>
            @error('block_reward')
                <div class="text-danger">{{ $message }}</div>
            @enderror
        </div>
