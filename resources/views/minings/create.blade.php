...
@section('title', 'Добавить запись о майнинге')

@section('content')
    <h1>Добавить запись о майнинге</h1>

    <form action="{{ route('minings.store') }}" method="POST">
        @csrf
        <div class="mb-3">
            ...
            <select name="mining_device_id" id="mining_device_id" class="form-control" required>
                ...
                @foreach ($miningDevices as $miningDevice)
                    <option value="{{ $miningDevice->id }}" {{ old('mining_device_id') == $miningDevice->id ? 'selected' : '' }}>{{ $miningDevice->device_name }}</option>
                @endforeach
            </select>
        ...
        <div class="mb-3">
            ...
            <select name="cryptocurrency_id" id="cryptocurrency_id" class="form-control" required>
                ...
                @foreach ($cryptocurrencies as $cryptocurrency)
                    <option value="{{ $cryptocurrency->id }}" {{ old('cryptocurrency_id') == $cryptocurrency->id ? 'selected' : '' }}>{{ $cryptocurrency->name }}</option>
                @endforeach
            </select>
        ...
        <div class="mb-3">
            ...
            <select name="algorithm_id" id="algorithm_id" class="form-control" required>
                ...
                @foreach ($algorithms as $algorithm)
                    <option value="{{ $algorithm->id }}" {{ old('algorithm_id') == $algorithm->id ? 'selected' : '' }}>{{ $algorithm->name }}</option>
                @endforeach
            </select>
            @error('algorithm_id')
                <div class="text-danger">{{ $message }}</div>
        ...
        <div class="mb-3">
            ...
            <input type="number" name="hashrate" id="hashrate" class="form-control" value="{{ old('hashrate') }}" step="0.01" min="0" required>
            @error('hashrate')
                <div class="text-danger">{{ $message }}</div>
            @enderror
        </div>
